...
def main(
  ...
  if config.checkpoint:
    ...
    if checkpoint_path is not None:
      ...
      # Also update the file shuffle seed
      if config.shuffle_seed is not None:
        shuffle_rng = jax.random.PRNGKey(config.shuffle_seed)
        config.shuffle_seed = int(
            jax.random.fold_in(shuffle_rng, global_step).sum()
        )
